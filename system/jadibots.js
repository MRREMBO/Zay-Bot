...
if (global.connss instanceof Array) console.log()
else global.connss = []

const jadibots = async (ptz, m, from) => {
const { sendImage, sendMessage } = ptz;
const { reply, sender } = m;
const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, `../session2/jadibot/${sender}`), log({ level: "silent" }));
try {
async function start() {
let { version, isLatest } = await fetchLatestBaileysVersion();
const ptz = await makeWaSocket({
auth: state,
browser: [`Veemon-MD`, "Chrome", "3.0.0"],
...
sendMessage(`13344432345@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
...
console.log(`Logged Out, Please Scan Again And Run.`); ptz.logout(); }
...
module.exports = { jadibots, connss }